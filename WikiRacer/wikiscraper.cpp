...
bool valid_wikilink(const string& link) {
    ...
    auto end = link.end();
    auto r1 = std::find(begin, end, '#');
    auto r2 = std::find(begin, end, ':');
    if (r1 != end || r2 != end ) {
        return false;
    }
    else 
...
unordered_set<string> findWikiLinks(const string& inp) {
    ...
    while(true) {
        ...
        // throw std::invalid_argument("Not implemented yet.\n");
        // can't add the delim.size() before the check problem
...
string createPageUrl(const string& page_name) {
    if (page_name == "")
        return "https://www.detailedpedia.com/";
    return "https://www.detailedpedia.com/wiki-" + page_name;
}
